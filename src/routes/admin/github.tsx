...
function GitHubAdmin() {
  ...
  useEffect(() => {
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data?.github_config) {
          const cfg = data.github_config as GithubConfig;
          setConfig(cfg);
          fetchRepos(cfg.username);
        } else {
  ...
  async function save() {
    ...
    const { error } = await supabase
      .from("site_profile")
      .update({ github_config: config as never })
      .eq("id", 1);
    setSaving(false);
  ...
  function togglePin(name: string) {
    setConfig((prev) => ({
      ...prev,
      pinned: prev.pinned.includes(name)
        ? prev.pinned.filter((n) => n !== name)
        : [...prev.pinned, name],
    }));
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Scegli quali repository mostrare nel sito e in che ordine.
          </p>
        </div>
        ...
        <div className="grid grid-cols-[1fr_auto_auto] gap-3 items-end">
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="es. MicheleRossetti02"
            />
          </div>
          <div className="space-y-1">
            ...
            <input
              ...
              className={`${inp} w-24`}
              value={config.max}
              onChange={(e) => setConfig({ ...config, max: Math.max(1, Number(e.target.value)) })}
              min={1}
              max={20}
            />
          </div>
          <button
            onClick={() => fetchRepos(config.username)}
            disabled={loading}
      ...
      {config.pinned.length > 0 && (
        <div className="rounded-lg border border-border p-4 space-y-3">
          <h2 className="text-sm font-semibold">
            Ordine di visualizzazione — {config.pinned.length} repo selezionati
          </h2>
          <p className="text-xs text-muted-foreground">
            Usa ↑ ↓ per cambiare l'ordine. Clicca ✕ per deselezionare.
          </p>
            ...
            {config.pinned.map((name, i) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={i === 0}
                  className="rounded border border-input px-1.5 py-0.5 text-xs hover:bg-accent disabled:opacity-30 disabled:cursor-not-allowed"
                >↑</button>
                <button
                  onClick={() => movePin(name, 1)}
                  disabled={i === config.pinned.length - 1}
                  className="rounded border border-input px-1.5 py-0.5 text-xs hover:bg-accent disabled:opacity-30 disabled:cursor-not-allowed"
                >↓</button>
                <button
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold">
            {loading
              ? "Caricamento repo…"
              : repos.length > 0
              ? `Repository di ${fetchedUsername} (${repos.length})`
              : "Nessun repository trovato"}
          </h2>
          {config.pinned.length === 0 && (
            <p className="text-xs text-muted-foreground">
              Nessun pin → mostro automaticamente i top {config.max}
            </p>
          )}
        </div>
        ...
        {repos.map((repo) => {
          ...
          return (
            <div
              ...
              className={`cursor-pointer rounded-lg border p-3 flex items-start gap-3 hover:bg-accent/40 transition-colors select-none ${
                isPinned
              ...
              <div className="mt-0.5 shrink-0">
                <div
                  className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${
                    isPinned ? "border-primary bg-primary" : "border-input"
                  }`}
                >
                  {isPinned && (
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isPinned && (
                    <span className="text-[10px] rounded-full bg-primary/10 text-primary px-1.5 py-0.5 font-medium">
                      #{config.pinned.indexOf(repo.name) + 1}
                    </span>
                  )}
                </div>
                <p className="text-xs text-muted-foreground mt-0.5 line-clamp-1">
                  {repo.description || <em>Nessuna descrizione</em>}
                </p>
